...
const GeniusForm = ({ isOpen, onClose, onSubmit, formData, setFormData, isEditing, validationErrors }) => {
    ...
    useEffect(() => {
        if (isOpen) { // Only update when modal is open
            if (isEditing && formData.image && typeof formData.image === 'string') {
                // Editing existing genius with an image
                setImagePreview({
                    url: `https://api.ditq.org/storage/${formData.image}`,
                    isExisting: true
                });
            } else if (formData.image instanceof File) {
                // Adding new genius or selected a new image while editing
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="text-xl font-semibold text-gray-900">
                                {isEditing ? 'تعديل النابغة' : 'إضافة نابغة جديد'}
                            </h3>
                            <p className="text-sm text-gray-500 mt-1">
                                {isEditing ? 'قم بتعديل بيانات النابغة' : 'أضف نابغة جديداً'}
                            </p>
                        </div>
                        <button
                            onClick={closeModal}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                className={`w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 placeholder-gray-400 ${validationErrors.name ? 'border-red-500' : ''}`}
                                placeholder="أدخل اسم النابغة"
                                required
                                disabled={loading}
                                dir="rtl"
                            />
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea
                                ...
                                placeholder="اكتب تفاصيل النابغة"
                                required
                                disabled={loading}
                                dir="rtl"
                            />
                            {validationErrors.details && (
                                <p className="text-sm text-red-600">{validationErrors.details[0]}</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            {validationErrors.image && (
                                <p className="text-sm text-red-600">{validationErrors.image[0]}</p>
                            )}
                        </div>
                ...
                <div className="sticky bottom-0 bg-gray-50 px-6 py-4 rounded-b-2xl border-t border-gray-100">
                    <div className="flex justify-end space-x-3 space-x-reverse">
                        ...
                        <button
                            ...
                            disabled={loading || !formData.name?.trim() || !formData.details?.trim()}
                        >
                            {loading ? (
                                <>
                                ...
                                <>
                                    {isEditing ? 'تحديث النابغة' : 'إضافة النابغة'}
                                </>
                            )}
                        </button>
                    </div>
